...
void loop()
{
  ...
  { 
    ...
    {
       find_distance();
       if(distance1<=35 && distance1>=15)  // if our hand detects in the right sensor before 300 milli seconds this condition becomes true. ( usually it happens if we swipe our hand from left to right sensor )
       {
         Serial.println("change");         // send "change" serially.
         ri=1;                              // store 1 in variable l. ( it avoids the program to enter into the upcoming if condition )
         break;                            // break the loop.
       }
    }

    if(ri==0)                               // this condition will become true, only if we swipe our hand in front of left sensor.
    {
    Serial.println("right");            // send "previous" serially.
    while(distance2<=35 && distance2>=15) // this loop will rotate untill we removes our hand infront of the left sensor. this will avoid not to enter this if condition again.
    find_distance();                      
    }
    ri=0; 
  ...
  { 
    ...
    {
    Serial.println("left");               // send "previous" serially.
